# herdr `[[build]]` step, Windows twin of install.sh: download the prebuilt herdr-glowr
...
# $HERDR_PLUGIN_ROOT\bin\herdr-glowr.exe.
...
# HERDR_GLOWR_BASE_URL is a test hook: point it at a local directory of staged assets and
# Get-Asset copies instead of downloading.
$Base = if ($env:HERDR_GLOWR_BASE_URL) { $env:HERDR_GLOWR_BASE_URL }
...
Write-Output "  1) install the agent skill:  npx skills add dcieslak19973/herdr-glowr --skill glowr-comments -g"
Write-Output "     (or: & '$(Join-Path $BinDir "$Name.exe")' skill-install . or: herdr plugin action invoke skill-install-windows --plugin dcieslak19973.glowr)"
