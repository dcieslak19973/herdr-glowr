#!/usr/bin/env bash
# herdr `[[build]]` step: download the prebuilt herdr-glowr binary for this platform from the
...
# runtime env). At runtime the pane command reads $HERDR_PLUGIN_ROOT/bin/herdr-glowr.
set -euo pipefail

NAME="herdr-glowr"
REPO="dcieslak19973/herdr-glowr"
...
# Link the binary onto PATH so `herdr-glowr ...` works directly in a shell (the sidebar pane
# and actions never need this — they invoke $HERDR_PLUGIN_ROOT/bin/herdr-glowr by absolute
...
echo "  1) install the agent skill:  npx skills add dcieslak19973/herdr-glowr --skill glowr-comments -g"
echo "     (or: $NAME skill-install · or: herdr plugin action invoke skill-install --plugin dcieslak19973.glowr)"
