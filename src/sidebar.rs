//! `herdr-glowr sidebar <toggle|open|close|auto-open>` — the sidebar actions and event hook.
//!
//! Rust port of reviewr's `sidebar.rs` (`docs/superpowers/specs/herdr-host.md`-equivalent
//! contracts A3/A5/A7, P5/P6): one implementation for every platform, no `bash` or `jq` at
//! runtime. The workspace's sidebar is any pane labeled "glowr" in the live pane list; there
//! is no state file. Actions refuse loudly (exit 1, one stderr line) and report successes on
...
/// Every pane id labeled "glowr" in a `pane list` snapshot — the workspace's sidebar,
...
fn sidebar_panes(panes_json: &str) -> Vec<String> {
    serde_json::from_str::<serde_json::Value>(panes_json)
        ...
        .map(|panes| {
            panes
                .iter()
                .filter(|p| p.get("label").and_then(serde_json::Value::as_str) == Some("glowr"))
...
/// `herdr-plugin.toml` prepend PATH for unix actions; this is the binary's own last resort,
/// for a Windows box without Git-for-Windows on PATH or any minimal-PATH environment the
/// manifest doesn't reach.
#[cfg(unix)]
...
fn open_sidebar(
    ...
    let plugin =
        env_non_empty("HERDR_PLUGIN_ID").unwrap_or_else(|| "dcieslak19973.glowr".to_string());
...
mod tests {
    ...
    #[test]
    fn sidebar_panes_selects_glowr_labels_only() {
        let json = r#"{"result":{"panes":[
            {"pane_id":"p1","label":"agent"},
            {"pane_id":"p2","label":"glowr"},
            {"pane_id":"p3","label":"glowr"}
        ]}}"#;
