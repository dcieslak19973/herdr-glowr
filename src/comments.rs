//! Persistent per-repo comment store: `<git-dir>/glowr/comments/<id>.json`.
//!
...
impl Store {
    /// Resolve `<git-dir>/glowr/comments` from `repo` via `git rev-parse --git-dir`
    ...
    pub fn open(repo: &Path) -> Result<Self, StoreError> {
        ...
        Ok(Self { dir: git_dir.join("glowr").join("comments") })
    }
    ...
    /// Persist a new comment (creates the dir on first write). Returns its fresh id.
    pub fn add(&self, comment: &Comment, author: Author) -> Result<String, StoreError> {
...
/// that as a corrupt file (skip, log, never delete).
...
mod tests {
    ...
    fn round_trips_a_comment_through_the_store() {
        ...
        let id = store.add(&sample_comment(), Author::User).unwrap();
        assert!(id.starts_with("c-"));
        let loaded = store.load();
        assert_eq!(loaded.len(), 1);
        assert_eq!(loaded[0].id, id);
        assert_eq!(loaded[0].comment, sample_comment());
    }
    ...
    fn set_status_preserves_unknown_fields() {
        let dir = tempfile::tempdir().unwrap();
        let store = Store::at(dir.path().join("comments"));
        let id = store.add(&sample_comment(), Author::Agent).unwrap();
        // Simulate a future writer adding a field.
        let path = dir.path().join("comments").join(format!("{id}.json"));
        let mut v: serde_json::Value =
            serde_json::from_str(&std::fs::read_to_string(&path).unwrap()).unwrap();
        v["future_field"] = serde_json::json!(42);
        std::fs::write(&path, serde_json::to_string(&v).unwrap()).unwrap();

        assert!(store.set_status(&id, Status::Resolved).unwrap());
    ...
    fn corrupt_files_are_skipped_not_deleted() {
        let dir = tempfile::tempdir().unwrap();
        let store = Store::at(dir.path().join("comments"));
        store.add(&sample_comment(), Author::User).unwrap();
    ...
    fn to_value_and_from_value_round_trip_and_reject_bad_json() {
        ...
        assert!(v.get("side").is_none(), "no side field is ever written");
    ...
    fn stored_comment_json_has_no_side() {
        ...
        let id = store
            ...
            .unwrap();
        let raw = std::fs::read_to_string(dir.path().join("comments").join(format!("{id}.json")))
            .unwrap();
