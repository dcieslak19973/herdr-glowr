//! `herdr-glowr comment …` and `herdr-glowr skill-path` — the agent-facing CLI.
//!
...
//! (no dependency) matching the contract in `docs/superpowers/specs/*.md`: an unknown flag or
//! a flag missing its value is a usage error (exit 2), a store failure (not a git repo, an
//! unknown id) is exit 1 with a single stderr line naming the problem, and success is exit 0
//! with the command's stdout (an id for `add`, rows or JSON for `list`, nothing otherwise).
...
/// `args[1]` is `"comment"`, `"skill-path"`, or `"skill-install"`. Takes the `Vec` by value
/// (the planned interface): `main` has no further use for argv, so ownership moves here.
...
fn open_store() -> Result<Store, ExitCode> {
    let cwd = std::env::current_dir().map_err(|e| {
        eprintln!("glowr: cannot read current directory: {e}");
        ExitCode::from(1)
    })?;
...
fn comment_add(args: &[String]) -> ExitCode {
    ...
    if start == 0 {
        eprintln!("glowr: --start must be >= 1");
        return ExitCode::from(2);
    }
    if end < start {
        eprintln!("glowr: --end must be >= --start");
        return ExitCode::from(2);
    }
    ...
    match store.add(&comment, author) {
        Ok(id) => {
            println!("{id}");
            ExitCode::SUCCESS
        }
...
fn comment_resolve(args: &[String]) -> ExitCode {
    ...
    match store.set_status(id, Status::Resolved) {
        ...
        Ok(false) => {
            eprintln!("glowr: no such comment: {id}");
            ExitCode::from(1)
        }
        Err(error) => {
            eprintln!("glowr: {error}");
...
/// `<plugin-root>/skills/glowr-comments/SKILL.md`, where `plugin-root` is the running
...
/// checkout rather than the packaged plugin). Returns the message (without the `glowr: `
...
fn resolve_skill_source() -> Result<PathBuf, String> {
    const REL: &str = "skills/glowr-comments/SKILL.md";
...
fn skill_path() -> ExitCode {
    match resolve_skill_source() {
        ...
        Err(message) => {
            eprintln!("glowr: {message}");
            ExitCode::from(1)
        }
    }
}

/// `$HOME/.claude/skills/glowr-comments` (`%USERPROFILE%` on Windows), the default
...
fn default_skill_install_target() -> Option<PathBuf> {
    std::env::var_os("HOME")
        ...
        .map(|home| Path::new(&home).join(".claude").join("skills").join("glowr-comments"))
}
...
fn print_installed(dest: &Path) {
    ...
    println!("  Plan/spec feedback happens in the herdr-glowr sidebar — when starting work or");
    println!(
        "  when review feedback is mentioned, run `herdr-glowr comment list` and address open comments."
    );
}
...
fn install_skill(source: &Path, dest: &Path, copy: bool) -> ExitCode {
    if !copy {
        ...
        eprintln!("glowr: symlink unavailable — copied; re-run after plugin updates");
    }
    match std::fs::copy(source, dest) {
        ...
        Err(error) => {
            eprintln!("glowr: cannot install to {}: {error}", dest.display());
            ExitCode::from(1)
        }
    }
}
...
/// `address my plan comments` works with no per-session reminder. See the module doc and
...
fn skill_install(args: &[String]) -> ExitCode {
    ...
    if project {
        let cwd = match std::env::current_dir() {
            Ok(cwd) => cwd,
            Err(error) => {
                eprintln!("glowr: cannot read current directory: {error}");
                return ExitCode::from(1);
            }
        };
        target = Some(cwd.join(".agents").join("skills").join("glowr-comments"));
    }

    let Some(target_dir) = target.or_else(default_skill_install_target) else {
        eprintln!(
            "glowr: cannot determine home directory (set HOME or USERPROFILE, or pass --target)"
        );
        return ExitCode::from(1);
    };
    ...
    if let Err(error) = std::fs::create_dir_all(&target_dir) {
        eprintln!("glowr: cannot create {}: {error}", target_dir.display());
        return ExitCode::from(1);
    }
    ...
    if let Ok(meta) = std::fs::symlink_metadata(&dest) {
        ...
        if !force {
            eprintln!(
                "glowr: {} already exists and differs from the bundled skill; re-run with --force to replace",
                dest.display()
            );
            return ExitCode::from(1);
        }
        if let Err(error) = std::fs::remove_file(&dest) {
            eprintln!("glowr: cannot remove existing {}: {error}", dest.display());
            return ExitCode::from(1);
        }
    }
