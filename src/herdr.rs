...
//!
//! See `docs/superpowers/specs/2026-08-13-herdr-glowr-design.md` ("herdr host integration").
//! Uses the herdr CLI via `$HERDR_BIN_PATH`. Only the agent-send export depends on this
//! module; browsing and clipboard do not. Unlike reviewr, glowr tracks no "since last agent
//! turn" baseline, so there is no agent-status polling here — only pane discovery and send.
...
/// parsing live, shared by pane resolution.
...
///
/// A refusal says why and names the clipboard fallback — the status line renders it as
/// `agent failed: <this message>`.
...
/// The sole agent in this tab, else the sole workspace agent.
///
...
mod tests {
    ...
    #[test]
    fn the_glowr_pane_excludes_itself_so_the_real_agent_resolves() {
    ...
    fn no_matching_agent_refuses_as_no_agent() {
        ...
        // An agent exists, but in another workspace entirely.
    ...
    fn two_tab_agents_refuse_as_several_even_without_a_workspace_id() {
        ...
        // still a several-agents refusal, not a missing-agent one.
