...
//!
//! The highlighter is rebuilt when the theme changes and produces per-line foreground
//! spans; the pane keeps the terminal's own background, so only token colors come from
//! the theme.
...
impl Highlighter {
    ...
    /// rather than crashing. Most files color out of the box via the broad two-face syntax
    /// set.
        ...
        for line in LinesWithEndings::from(content) {
            let spans = match h.highlight_line(line, syntaxes) {
                Ok(regions) => regions
                    ...
                    .collect(),
                // A grammar error degrades to plain text rather than blocking the render.
                Err(_) => vec![Span {
...
mod tests {
    ...
    fn bundled_syntax_themes_all_parse() {
        ...
        // A loaded theme tokenizes rust into more than one span; a failed load would yield a
        // single plain span — so this guards the parse path for every bundled theme, the only
        // `SyntaxChoice` that can fail.
